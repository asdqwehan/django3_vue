...
def show_headers(request):
    ...
    try:
        ...
        response['msg'] = json.loads(serializers.serialize('json', headers))
        response['code'] = 0
    except Exception as e:
        response['msg'] = str(e)
        response['code'] = 1
...
def add_headers(request):
    ...
    if request.method == "POST":
        try:
            headers_list = json.loads(request.body)
            print(headers_list['user_agent'])
            HeadersG.objects.create(
                user_agent = headers_list['user_agent'],
                content_type = headers_list['content_type'],
                token = headers_list['token']
            )
            response['msg'] = '添加成功'
            response['code'] = 0
        except Exception as e:
